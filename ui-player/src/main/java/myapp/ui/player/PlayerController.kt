...
class PlayerController(val context: Context) : IVLCVout.Callback, MediaPlayer.EventListener, CoroutineScope {
    ...
    internal suspend fun startPlayback(media: IMedia, listener: MediaPlayerEventListener, time: Long) {
        ...
        withContext(Dispatchers.IO) {
            if (!mediaplayer.isReleased) mediaplayer.media = media.apply { if (hasRenderer) parse() }
        }
    ...
    fun getVideoTracksCount() =
        if (!mediaplayer.isReleased && mediaplayer.hasMedia()) mediaplayer.videoTracksCount else 0

    fun getVideoTracks(): Array<out MediaPlayer.TrackDescription>? =
        if (!mediaplayer.isReleased && mediaplayer.hasMedia()) mediaplayer.videoTracks else emptyArray()

    fun getVideoTrack() = if (!mediaplayer.isReleased && mediaplayer.hasMedia()) mediaplayer.videoTrack else -1

    fun getCurrentVideoTrack(): IMedia.VideoTrack? =
        if (!mediaplayer.isReleased && mediaplayer.hasMedia()) mediaplayer.currentVideoTrack else null

    fun getAudioTracksCount() =
        if (!mediaplayer.isReleased && mediaplayer.hasMedia()) mediaplayer.audioTracksCount else 0

    fun getAudioTracks(): Array<out MediaPlayer.TrackDescription>? =
        if (!mediaplayer.isReleased && mediaplayer.hasMedia()) mediaplayer.audioTracks else emptyArray()

    fun getAudioTrack() = if (!mediaplayer.isReleased && mediaplayer.hasMedia()) mediaplayer.audioTrack else -1

    fun setVideoTrack(index: Int) =
        !mediaplayer.isReleased && mediaplayer.hasMedia() && mediaplayer.setVideoTrack(index)

    fun setAudioTrack(index: Int) =
        !mediaplayer.isReleased && mediaplayer.hasMedia() && mediaplayer.setAudioTrack(index)
    ...
    private fun newMediaPlayer(): MediaPlayer {
        return MediaPlayer(VLCInstance.getInstance(context)).apply {
            ...
            setAudioDigitalOutputEnabled(true)
    ...
    fun setRate(rate: Float, save: Boolean) {
        ...
        speed.value = rate
        if (save && settings.getBoolean(
                if (isVideoPlaying()) KEY_PLAYBACK_SPEED_PERSIST_VIDEO else KEY_PLAYBACK_SPEED_PERSIST,
                false
            )
        )
...
//    }
    ...
    fun updateViewpoint(yaw: Float, pitch: Float, roll: Float, fov: Float, absolute: Boolean) =
        mediaplayer.updateViewpoint(yaw, pitch, roll, fov, absolute)

    fun navigate(where: Int) = mediaplayer.navigate(where)

    fun getChapters(title: Int): Array<out MediaPlayer.Chapter>? =
        if (!mediaplayer.isReleased) mediaplayer.getChapters(title) else emptyArray()
    ...
    private fun onPlayerError() {
        launch(Dispatchers.Main) {
            restart()
            Toast.makeText(context, "Player crashed", Toast.LENGTH_LONG).show()
        }
    }

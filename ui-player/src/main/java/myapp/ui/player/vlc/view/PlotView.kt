...
import myapp.ui.player.BuildConfig
            ...
            if (lineGraph.index == index) {
                ...
                data.forEach { lineGraph ->
                    listenerValue.add(
                        Pair(
                            lineGraph,
                            "${String.format("%.0f", lineGraph.data[lineGraph.data.keys.maxOrNull()])} kb/s"
                        )
                    )
                }
    ...
    private fun drawGrid(canvas: Canvas?, maxY: Float, minX: Long, maxX: Long) {
        canvas?.let {
            ...
            while (index > minX) {
                ...
                it.drawText(
                    formattedText,
                    xCoord - (textPaint.measureText(formattedText) / 2),
...
//    }
    ...
    private fun getCoordinates(
        ...
        (measuredWidth * ((point.key - minX).toDouble() / (maxX - minX).toDouble())).toFloat(),
        measuredHeight * ((maxY - point.value) / maxY)
    )
